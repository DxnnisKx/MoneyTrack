...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNpYXRzbXNsaGp6cXR0ZHhmc2hlIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTkzODUyMjgsImV4cCI6MjA3NDk2MTIyOH0.OSNl7pwyVnta3QSJPA93lZtC5kHKlhNq87bC9tUKqu0';
...
function addUserInfo() {
    ...
    if (navContainer && currentUser) {
        ...
        if (!document.querySelector('.user-info')) {
            ...
            userInfo.style.cssText = 'display: flex; align-items: center; gap: 1rem; margin-left: auto; margin-right: 1rem;';
            userInfo.innerHTML = `
            ...
            navContainer.insertBefore(userInfo, navContainer.querySelector('.nav-menu'));
        }
    }
    ...
    if (navMenu && !document.querySelector('.sign-out-btn')) {
        ...
        signOutBtn.innerHTML = '<i class="bi bi-box-arrow-right"></i><span class="nav-text">Sign Out</span>';
...
window.setType = function(type) {
    ...
    submitBtn.style.background = type === 'income' ? 'var(--success)' : 'var(--danger)';
}
    ...
    historyList.innerHTML = transactions.map(t => {
        ...
        return `
            <div class="transaction-item ${className}">
                <div class="transaction-info">
                    <span class="transaction-desc">${t.description}</span>
                    <span class="transaction-date">${new Date(t.date).toLocaleDateString()} ${new Date(t.date).toLocaleTimeString()}</span>
                </div>
                <span class="transaction-amount ${className}">
                    ${sign}$${t.amount.toFixed(2)}
                </span>
...
function initCharts() {
    ...
    Chart.defaults.borderColor = getComputedStyle(document.body).getPropertyValue('--border');
}
